...
TEST_CASE("Filesystem Subsystem operations", "[subsystems][filesystem]") {
    ...
    auto* fs_api = ecs.try_get<filesystem_service>();

    std::filesystem::path test_dir = std::filesystem::current_path() / "test_mount";
    std::filesystem::create_directories(test_dir);

    SECTION("Mounting a valid OS path succeeds") {
        auto res = fs_api->mount(fs_api->instance, test_dir.string().c_str(), "mount://test", true);
        REQUIRE(res == 0);
    }

    SECTION("Attempting to write to a read_only mount returns error") {
        fs_api->mount(fs_api->instance, test_dir.string().c_str(), "mount://test_ro", true);
        uint8_t dummy = 'A';
        auto res = fs_api->write(fs_api->instance, "mount://test_ro/file.txt", &dummy, 1, false);
        REQUIRE(res != 0);
    }

    SECTION("Path Traversal Protection: Writing to mount://cache/../../../etc/passwd is caught and returns an error") {
        fs_api->mount(fs_api->instance, test_dir.string().c_str(), "mount://cache", false);
        uint8_t dummy = 'A';
        auto res = fs_api->write(fs_api->instance, "mount://cache/../../../etc/passwd", &dummy, 1, false);
        REQUIRE(res != 0);
    }
    ...
    SECTION("VFS Fallback: Reading a file existing in mount://app succeeds even if missing in mount://cache") {
        ...
        fs_api->mount(fs_api->instance, test_dir.string().c_str(), "mount://cache", false);
        fs_api->mount(fs_api->instance, app_dir.string().c_str(), "mount://app", true);

        sandbox_payload payload{};
        auto res = fs_api->read(fs_api->instance, "mount://app/test.txt", &payload);
        REQUIRE(res == 0);
    ...
    SECTION("Raw C-ABI methods return correct FlatBuffer payloads") {
        ...
        uint8_t data[] = { 'Z', 'Z' };
        REQUIRE(fs_api->write(fs_api->instance, "mount://cache/macro.txt", data, 2, false) == 0);

        // state query using raw API
        sandbox_payload state_payload{};
        REQUIRE(fs_api->state(fs_api->instance, "mount://cache/macro.txt", &state_payload) == 0);
        ...
        REQUIRE(fs_api->list(fs_api->instance, "mount://cache", false, &list_payload) == 0);
