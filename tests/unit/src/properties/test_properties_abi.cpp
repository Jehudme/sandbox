...
TEST_CASE("PropABI: lifecycle (create/destroy)", "[properties][abi][lifecycle]")
{
    SECTION("create returns non-null") {
        ...
        REQUIRE(!SANDBOX_HANDLE_IS_INVALID(h));
...
TEST_CASE("PropABI: load and dump", "[properties][abi][load_dump]")
{
    ...
    SECTION("load with null handle returns false") {
        ...
        REQUIRE(sandbox_properties_load(sandbox_handle_t{0}, json.c_str(), json.size(), SANDBOX_FORMAT_JSON) == false);
    }
    ...
    SECTION("dump with null handle returns null") {
        REQUIRE(sandbox_properties_dump(sandbox_handle_t{0}, SANDBOX_FORMAT_JSON) == nullptr);
    }
...
TEST_CASE("PropABI: merge and sub", "[properties][abi][merge_sub]")
{
    SECTION("merge copies source at given path") {
        sandbox_handle_t dst{0};
        sandbox_properties_create(&dst);
        sandbox_handle_t src{0};
        sandbox_properties_create(&src);
    ...
    SECTION("sub extracts a sub-tree") {
        sandbox_handle_t parent{0};
        sandbox_properties_create(&parent);
        sandbox_properties_set_int64(parent, "config/timeout", 30);
        sandbox_handle_t sub{0};
        REQUIRE(sandbox_properties_sub(parent, "config", &sub) == SANDBOX_STATUS_SUCCESS);
        REQUIRE(!SANDBOX_HANDLE_IS_INVALID(sub));
        int64_t timeout = 0;
    ...
    SECTION("sub with null handle returns null") {
        sandbox_handle_t sub{0};
        REQUIRE(sandbox_properties_sub(sandbox_handle_t{0}, "any", &sub) == SANDBOX_STATUS_ERROR_INVALID);
    }
}
...
TEST_CASE("PropABI: null safety", "[properties][abi][null_safety]")
{
    SECTION("setters with null handle are safe no-ops") {
        REQUIRE_NOTHROW(sandbox_properties_set_int64(sandbox_handle_t{0}, "k", 1));
        REQUIRE_NOTHROW(sandbox_properties_set_double(sandbox_handle_t{0}, "k", 1.0));
        REQUIRE_NOTHROW(sandbox_properties_set_bool(sandbox_handle_t{0}, "k", true));
        REQUIRE_NOTHROW(sandbox_properties_set_string(sandbox_handle_t{0}, "k", "v"));
    }

    SECTION("getters with null handle return false/null") {
        int64_t i = 0; double d = 0.0; bool b = false;
        REQUIRE(sandbox_properties_get_int64(sandbox_handle_t{0}, "k", &i)    == false);
        REQUIRE(sandbox_properties_get_double(sandbox_handle_t{0}, "k", &d)   == false);
        REQUIRE(sandbox_properties_get_bool(sandbox_handle_t{0}, "k", &b)     == false);
        bool was_null = false;
        sandbox_properties_read_string(sandbox_handle_t{0}, "k", [](const char* val, void* ctx) {
            *static_cast<bool*>(ctx) = (val == nullptr);
        }, &was_null);
        REQUIRE(was_null == true);
    }
