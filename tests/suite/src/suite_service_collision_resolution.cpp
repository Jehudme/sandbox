...
static ServiceInfo make_svc(const char* name, int major, int minor) {
    ServiceInfo s{};
...
static ModuleInfo make_mod(const char* name, int major, int minor, int patch,
                            const ServiceInfo* svc, void (*fn)(ecs_world_t*) = nullptr) {
    ModuleInfo m{};
...
TEST_CASE("Suite: Service collision — winner selection", "[suite][collision]")
{
    SECTION("two-way: higher module version wins") {
        ...
        winner = loser = 0;
        ServiceInfo svc = make_svc("IPhysicsEngine", 1, 0);
        ...
        Bootstrapper::stage_module(v1);
        Bootstrapper::stage_module(v2);
        Bootstrapper b;
    ...
    SECTION("three-way: highest version among three wins") {
        ...
        ServiceInfo svc = make_svc("IAudioEngine", 1, 0);
        ...
        Bootstrapper::stage_module(fmod);
        Bootstrapper::stage_module(openal);
        Bootstrapper::stage_module(xaudio);
        Bootstrapper b;
    ...
    SECTION("auto-resolved provider loses to explicit higher-version provider") {
        ...
        ServiceInfo svc = make_svc("INetworkEngine", 1, 0);
...
TEST_CASE("Suite: No collision between different services", "[suite][collision]")
{
    ...
    ServiceInfo rend_svc  = make_svc("IRenderer", 1, 0);
    ServiceInfo audio_svc = make_svc("IAudio",    1, 0);
    ...
    Bootstrapper::stage_module(rend);
    Bootstrapper::stage_module(aud);
    Bootstrapper b;
