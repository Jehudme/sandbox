...
namespace sandbox::core {
    ...
    void engine_t::save_bootstrapper() {
        ...
        bootstrapper_component.internal_bootstrapper = reinterpret_cast<sandbox_bootstrapper_t*>(m_bootstrapper.get());
        ecs_set_id(entity_world.c_ptr(), ecs_id(sandbox_bootstrapper_component_t), ecs_id(sandbox_bootstrapper_component_t), sizeof(sandbox_bootstrapper_component_t), &bootstrapper_component);
    }
    ...
    void engine_t::boot() {
        ...
        if (m_arguments->has({"engine", "libraries"})) {
            if (auto libraries = m_arguments->get<std::vector<std::string>>({"engine", "libraries"})) {
                for (const auto& library_path : *libraries) {
                    try {
                        bootstrapper_t::index_library(entity_world, library_path);
                    } catch (const library_load_error& e) {
                        std::cerr << "Library load error skipped: " << e.what() << "\n";
                        sandbox::modules::logs::warn(entity_world, "Library load error skipped: {}", e.what());
                    } catch (const std::exception& e) {
                        std::cerr << "Unexpected error indexing library '" << library_path << "': " << e.what() << "\n";
                        sandbox::modules::logs::error(entity_world, "Unexpected error indexing library '{}': {}", library_path, e.what());
                    }
                }
            }
        }
        ...
        if (m_arguments->has({"engine", "sandbox"})) {
            if (auto modules = m_arguments->get<std::vector<std::string>>({"engine", "sandbox"})) {
                for (const auto& module_name : *modules) {
                    try {
                        m_bootstrapper->activate(entity_world, module_name);
                    } catch (const module_activation_error& e) {
                        std::cerr << "Failed to activate module '" << module_name << "': " << e.what() << "\n";
                        sandbox::modules::logs::warn(entity_world, "Failed to activate module '{}': {}", module_name, e.what());
                    } catch (const std::exception& e) {
                        std::cerr << "Unexpected error activating module '" << module_name << "': " << e.what() << "\n";
                        sandbox::modules::logs::error(entity_world, "Unexpected error activating module '{}': {}", module_name, e.what());
                    }
                }
            }
        }
