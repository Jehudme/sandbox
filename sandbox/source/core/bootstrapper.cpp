...
auto safe_view = [](const char *str) {
  return str ? std::string_view(str) : std::string_view();
};


void bootstrapper_t::load_library(flecs::world &entity_world,
...
void bootstrapper_t::stage_service(const service_info_t &info) {
  auto it = std::find_if(
      m_services.begin(), m_services.end(), [&](const service_info_t &service) {
...
void bootstrapper_t::stage_module(const module_info_t &info) {
  auto it = std::find_if(
      m_modules.begin(), m_modules.end(), [&](const module_info_t &module) {
...
find_best_module(std::string_view name, std::string_view architecture,
                 int version_major, int version_minor, int version_patch,
                 bool exact_patch, const std::vector<module_info_t> &modules) {
  const module_info_t *best_match = nullptr;
  for (const auto &module_info : modules) {
    if (safe_view(module_info.name) != name)
      continue;
    if (safe_view(module_info.architecture) != architecture)
      continue;
    if (version_major > 0 && module_info.version_major != version_major)
      continue;
    if (version_minor >= 0 && module_info.version_minor < version_minor)
      continue;
    if (exact_patch && module_info.version_patch != version_patch)
      continue;
    ...
    if (!best_match) {
      ...
      if (module_info.version_major != best_match->version_major) {
        ...
        if (module_info.version_minor > best_match->version_minor)
          best_match = &module_info;
      } else if (module_info.version_patch != best_match->version_patch) {
        if (module_info.version_patch > best_match->version_patch)
          best_match = &module_info;
...
static const module_info_t *
find_best_service_provider(std::string_view service_name,
                           std::string_view architecture, int version_major,
                           int version_minor,
                           const std::vector<module_info_t> &modules) {
  const module_info_t *best_match = nullptr;
  for (const auto &module_info : modules) {
    if (!module_info.service)
      continue;
    if (safe_view(module_info.service->name) != service_name)
      continue;
    if (safe_view(module_info.architecture) != architecture)
      continue;
    if (version_major > 0 &&
        module_info.service->version_major != version_major)
      continue;
    if (version_minor >= 0 &&
        module_info.service->version_minor < version_minor)
      continue;

    if (!best_match) {
      best_match = &module_info;
    } else {
      if (module_info.service->version_major !=
          best_match->service->version_major) {
        if (module_info.service->version_major >
            best_match->service->version_major)
          best_match = &module_info;
      } else if (module_info.service->version_minor !=
                 best_match->service->version_minor) {
        if (module_info.service->version_minor >
            best_match->service->version_minor)
          best_match = &module_info;
...
void bootstrapper_t::activate(flecs::world &entity_world,
                              std::string_view module_urn) {
  size_t at_position = module_urn.find('@');
  if (at_position == std::string_view::npos) {
    std::cerr << std::format("Invalid module string format (missing @): {}\n", module_urn);
    return;
  }
  ...
  if (dash_position == std::string_view::npos) {
    std::cerr << std::format("Invalid module string format (missing architecture-name separator): {}\n", module_urn);
    return;
  }
  ...
  auto parse_integer = [](std::string_view str, int default_value) {
    ...
    if (result.ec != std::errc())
      throw module_activation_error(
          std::format("Invalid version number: {}", str));
    return value;
  };
  ...
  if (dot_first != std::string_view::npos) {
    ...
    if (dot_second != std::string_view::npos) {
      version_minor = parse_integer(
          version_str.substr(dot_first + 1, dot_second - dot_first - 1), 0);
  ...
  activate(entity_world, architecture, name, version_major, version_minor,
           version_patch);
}

void bootstrapper_t::activate(flecs::world &entity_world,
                              std::string_view architecture,
                              std::string_view name, int version_major,
                              int version_minor, int version_patch) {
  bool exact_patch = (version_patch >= 0);
  const module_info_t *best =
      find_best_module(name, architecture, version_major, version_minor,
                       version_patch, exact_patch, m_modules);
  if (best) {
    auto is_same_mod = [&](const module_info_t &m) {
      return std::strcmp(m.name, best->name) == 0 &&
             std::strcmp(m.architecture, best->architecture) == 0;
    };

    auto it = std::find_if(m_active_modules.begin(), m_active_modules.end(),
                           is_same_mod);
    auto booted_it = std::find_if(m_booted_modules.begin(),
                                  m_booted_modules.end(), is_same_mod);

    if (it == m_active_modules.end() && booted_it == m_booted_modules.end()) {
      m_active_modules.push_back(*best);
      sandbox::modules::logs::trace(
          entity_world, "Module staged for activation: {} v{}.{} (arch: {})",
          name, version_major, version_minor, architecture);
    }
  } else {
    std::string error_message = std::format(
        "No matching module found to activate: {} v{}.{} (arch: {})", name,
        version_major, version_minor, architecture);
    throw module_activation_error(std::format(
        "No matching module found to activate: {} v{}.{} (arch: {})", name,
        version_major, version_minor, architecture));
  }
}
...
void bootstrapper_t::boot(flecs::world &entity_world) {
  ...
  for (const auto &booted : m_booted_modules) {
    if (booted.service && booted.service->version_major > 0) {
      service_version_locks[booted.service->name] =
          booted.service->version_major;
    }
  }
  ...
  while (!m_active_modules.empty()) {
    ...
    while (processed_index < m_active_modules.size()) {
      ...
      for (size_t i = 0; i < current.requirement_count; ++i) {
        ...
        if (requirement.strictness != SANDBOX_REQUIREMENT_STRICTNESS_REQUIRED)
          continue;
        ...
        if (requirement.kind == SANDBOX_REQUIREMENT_KIND_SERVICE) {
          if (requirement.version_major > 0) {
            ...
            if (lock_it != service_version_locks.end() &&
                lock_it->second != requirement.version_major) {
              throw std::runtime_error(
                  std::format("Fatal Major Version Collision for service: {}",
                              requirement.name));
            }
          ...
          for (const auto &active : m_active_modules) {
            if (active.service &&
                safe_view(active.service->name) == requirement.name) {
              fulfilled = true;
              break;
            }
          }
          if (!fulfilled) {
            for (const auto &booted : m_booted_modules) {
              if (booted.service &&
                  safe_view(booted.service->name) == requirement.name) {
                fulfilled = true;
                break;
              }
            }
          ...
          if (!fulfilled) {
            const module_info_t *provider = find_best_service_provider(
                requirement.name, requirement.architecture,
                requirement.version_major, requirement.version_minor,
                m_modules);
            if (!provider) {
              throw module_dependency_error(std::format(
                  "Required service not found: {}", requirement.name));
            }
            sandbox::modules::logs::trace(
                entity_world,
                "Auto-pulled provider '{}' for required service: {}",
          ...
          if (!fulfilled) {
            ...
            const module_info_t *provider = find_best_module(
                requirement.name, requirement.architecture,
                requirement.version_major, requirement.version_minor,
                requirement.version_patch, exact, m_modules);
            if (!provider) {
              throw module_dependency_error(std::format(
                  "Required module not found: {}", requirement.name));
            }
            sandbox::modules::logs::trace(
                entity_world,
                "Auto-pulled module '{}' to satisfy dependency for: {}",
    ...
    while (processed_index < m_active_modules.size()) {
      ...
      for (size_t i = 0; i < current.requirement_count; ++i) {
        const auto &requirement = current.requirements[i];
        if (requirement.strictness != SANDBOX_REQUIREMENT_STRICTNESS_EXPECTED)
          continue;
    ...
    for (const auto &pair : service_providers) {
      ...
      if (indices.size() > 1) {
        ...
        for (size_t i = 1; i < indices.size(); ++i) {
          ...
          if (current.version_major != winner.version_major)
            current_wins = current.version_major > winner.version_major;
          else if (current.version_minor != winner.version_minor)
            current_wins = current.version_minor > winner.version_minor;
          else if (current.version_patch != winner.version_patch)
            current_wins = current.version_patch > winner.version_patch;
          else
            current_wins = safe_view(current.name) < safe_view(winner.name);
    ...
    for (size_t idx : to_evict) {
      sandbox::modules::logs::warn(entity_world,
                                   "ECS registration suppressed for module "
                                   "'{}' due to service collision on '{}'",
                                   m_active_modules[idx].name,
                                   m_active_modules[idx].service->name);
      m_active_modules.erase(m_active_modules.begin() + idx);
    }

    // Extract the current batch. This clears m_active_modules so that
    // any init_fn calling activate() will safely stage modules for the NEXT
    // while iteration.
    ...
    for (size_t a = 0; a < current_batch.size(); ++a) {
      ...
      for (size_t i = 0; i < mod_a.requirement_count; ++i) {
        ...
        // (If not found, it implies it was already resolved in a prior batch
        // and booted).
    ...
    while (!s.empty()) {
      ...
      if (current_batch[curr].init_fn) {
        std::cout << "[Bootstrapper] Booting module: "
                  << current_batch[curr].name << "\n";
    ...
    for (size_t i = 0; i < in_degree.size(); i++) {
      if (in_degree[i] != 0) {
        throw module_dependency_error(
            "Cyclic dependency detected during module boot.");
      }
    }
  }
}
