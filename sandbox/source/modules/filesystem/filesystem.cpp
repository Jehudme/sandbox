...
namespace sandbox::modules {
    ...
    filesystem_module::filesystem_module(world& ecs) {
        ...
        svc.instance = this;
        svc.execute_command = [](void* inst, uint32_t command_id, const uint8_t* payload, size_t size) {
            auto fs = static_cast<filesystem_module*>(inst);
            using namespace sandbox::schemas::filesystem;
            switch(static_cast<FilesystemCommand>(command_id)) {
                case FilesystemCommand_Mount: {
                    auto args = flatbuffers::GetRoot<MountArgs>(payload);
                    int32_t res = fs->mount(args->physical_path()->c_str(), args->virtual_prefix()->c_str(), args->read_only());
                    if (args->out_result_ptr()) *reinterpret_cast<int32_t*>(args->out_result_ptr()) = res;
                    break;
                }
                case FilesystemCommand_Unmount: {
                    auto args = flatbuffers::GetRoot<UnmountArgs>(payload);
                    int32_t res = fs->unmount(args->virtual_prefix()->c_str());
                    if (args->out_result_ptr()) *reinterpret_cast<int32_t*>(args->out_result_ptr()) = res;
                    break;
                }
                case FilesystemCommand_Read: {
                    auto args = flatbuffers::GetRoot<ReadArgs>(payload);
                    int32_t res = fs->read(args->virtual_path()->c_str(), reinterpret_cast<sandbox_payload*>(args->out_payload_ptr()));
                    if (args->out_result_ptr()) *reinterpret_cast<int32_t*>(args->out_result_ptr()) = res;
                    break;
                }
                case FilesystemCommand_Write: {
                    auto args = flatbuffers::GetRoot<WriteArgs>(payload);
                    int32_t res = fs->write(args->virtual_path()->c_str(), args->data()->data(), args->data()->size(), args->append());
                    if (args->out_result_ptr()) *reinterpret_cast<int32_t*>(args->out_result_ptr()) = res;
                    break;
                }
                case FilesystemCommand_List: {
                    auto args = flatbuffers::GetRoot<ListArgs>(payload);
                    int32_t res = fs->list(args->virtual_path()->c_str(), args->recursive(), reinterpret_cast<sandbox_payload*>(args->out_payload_ptr()));
                    if (args->out_result_ptr()) *reinterpret_cast<int32_t*>(args->out_result_ptr()) = res;
                    break;
                }
                case FilesystemCommand_Remove: {
                    auto args = flatbuffers::GetRoot<RemoveArgs>(payload);
                    int32_t res = fs->remove(args->virtual_path()->c_str());
                    if (args->out_result_ptr()) *reinterpret_cast<int32_t*>(args->out_result_ptr()) = res;
                    break;
                }
                case FilesystemCommand_Mkdir: {
                    auto args = flatbuffers::GetRoot<MkdirArgs>(payload);
                    int32_t res = fs->mkdir(args->virtual_path()->c_str());
                    if (args->out_result_ptr()) *reinterpret_cast<int32_t*>(args->out_result_ptr()) = res;
                    break;
                }
                case FilesystemCommand_Rename: {
                    auto args = flatbuffers::GetRoot<RenameArgs>(payload);
                    int32_t res = fs->rename(args->old_virtual_path()->c_str(), args->new_virtual_path()->c_str());
                    if (args->out_result_ptr()) *reinterpret_cast<int32_t*>(args->out_result_ptr()) = res;
                    break;
                }
                case FilesystemCommand_Copy: {
                    auto args = flatbuffers::GetRoot<CopyArgs>(payload);
                    int32_t res = fs->copy(args->source_virtual_path()->c_str(), args->destination_virtual_path()->c_str());
                    if (args->out_result_ptr()) *reinterpret_cast<int32_t*>(args->out_result_ptr()) = res;
                    break;
                }
                case FilesystemCommand_Move: {
                    auto args = flatbuffers::GetRoot<MoveArgs>(payload);
                    int32_t res = fs->move(args->source_virtual_path()->c_str(), args->destination_virtual_path()->c_str());
                    if (args->out_result_ptr()) *reinterpret_cast<int32_t*>(args->out_result_ptr()) = res;
                    break;
                }
                case FilesystemCommand_State: {
                    auto args = flatbuffers::GetRoot<StateArgs>(payload);
                    int32_t res = fs->state(args->virtual_path()->c_str(), reinterpret_cast<sandbox_payload*>(args->out_payload_ptr()));
                    if (args->out_result_ptr()) *reinterpret_cast<int32_t*>(args->out_result_ptr()) = res;
                    break;
                }
                case FilesystemCommand_Absolute: {
                    auto args = flatbuffers::GetRoot<AbsoluteArgs>(payload);
                    int32_t res = fs->absolute(args->virtual_path()->c_str(), reinterpret_cast<sandbox_payload*>(args->out_payload_ptr()));
                    if (args->out_result_ptr()) *reinterpret_cast<int32_t*>(args->out_result_ptr()) = res;
                    break;
                }
                case FilesystemCommand_SetProperty: {
                    auto args = flatbuffers::GetRoot<SetPropertyArgs>(payload);
                    fs->set_property(args->key()->c_str(), args->json_value()->c_str());
                    break;
                }
                case FilesystemCommand_GetProperty: {
                    auto args = flatbuffers::GetRoot<GetPropertyArgs>(payload);
                    int32_t res = fs->get_property(args->key()->c_str(), reinterpret_cast<sandbox_payload*>(args->out_payload_ptr()));
                    if (args->out_result_ptr()) *reinterpret_cast<int32_t*>(args->out_result_ptr()) = res;
                    break;
                }
            }
    ...
    std::expected<sandbox::schemas::filesystem::FileMetadataT, std::string> filesystem_module::state_impl(std::string_view virtual_path) const {
        std::string path = get_physfs_path(virtual_path);

        sandbox::schemas::filesystem::FileMetadataT metadata{};
        ...
        switch (stat.filetype) {
            case PHYSFS_FILETYPE_REGULAR:   metadata.type = sandbox::schemas::filesystem::FileType_File;      break;
            case PHYSFS_FILETYPE_DIRECTORY: metadata.type = sandbox::schemas::filesystem::FileType_Directory; break;
            case PHYSFS_FILETYPE_SYMLINK:   metadata.type = sandbox::schemas::filesystem::FileType_Symlink;   break;
            default:                        metadata.type = sandbox::schemas::filesystem::FileType_Other;     break;
        }
    ...
    int32_t filesystem_module::list(const char* virtual_path, bool recursive, sandbox_payload* out_payload) const {
        ...
        sandbox::schemas::filesystem::StringListBuilder slb(builder);
    ...
    int32_t filesystem_module::state(const char* virtual_path, sandbox_payload* out_payload) const {
        ...
        flatbuffers::FlatBufferBuilder builder;
        auto fb_offset = sandbox::schemas::filesystem::FileMetadata::Pack(builder, &res.value());
