...
namespace sandbox::modules {
    ...
    runner::runner(world& ecs) {
        ...
        svc.execute_command = [](void* inst, uint32_t command_id, const uint8_t* payload, size_t size) {
            auto run_inst = static_cast<runner*>(inst);
            using namespace sandbox::schemas::runner;
            switch(static_cast<RunnerCommand>(command_id)) {
                case RunnerCommand_StartAsync: {
                    auto args = flatbuffers::GetRoot<StartAsyncArgs>(payload);
                    int32_t res = run_inst->start_async(*reinterpret_cast<flecs::world*>(args->ecs_world_ptr()));
                    if (args->out_result_ptr()) *reinterpret_cast<int32_t*>(args->out_result_ptr()) = res;
                    break;
                }
                case RunnerCommand_RunSync: {
                    auto args = flatbuffers::GetRoot<RunSyncArgs>(payload);
                    int32_t res = run_inst->run_sync(*reinterpret_cast<flecs::world*>(args->ecs_world_ptr()));
                    if (args->out_result_ptr()) *reinterpret_cast<int32_t*>(args->out_result_ptr()) = res;
                    break;
                }
                case RunnerCommand_Quit: {
                    auto args = flatbuffers::GetRoot<QuitArgs>(payload);
                    int32_t res = run_inst->quit();
                    if (args->out_result_ptr()) *reinterpret_cast<int32_t*>(args->out_result_ptr()) = res;
                    break;
                }
                case RunnerCommand_Pause: {
                    auto args = flatbuffers::GetRoot<PauseArgs>(payload);
                    int32_t res = run_inst->pause();
                    if (args->out_result_ptr()) *reinterpret_cast<int32_t*>(args->out_result_ptr()) = res;
                    break;
                }
                case RunnerCommand_Resume: {
                    auto args = flatbuffers::GetRoot<ResumeArgs>(payload);
                    int32_t res = run_inst->resume();
                    if (args->out_result_ptr()) *reinterpret_cast<int32_t*>(args->out_result_ptr()) = res;
                    break;
                }
                case RunnerCommand_SetProperty: {
                    auto args = flatbuffers::GetRoot<SetPropertyArgs>(payload);
                    run_inst->set_property(args->key()->c_str(), args->json_value()->c_str());
                    break;
                }
                case RunnerCommand_GetProperty: {
                    auto args = flatbuffers::GetRoot<GetPropertyArgs>(payload);
                    int32_t res = run_inst->get_property(args->key()->c_str(), reinterpret_cast<sandbox_payload*>(args->out_payload_ptr()));
                    if (args->out_result_ptr()) *reinterpret_cast<int32_t*>(args->out_result_ptr()) = res;
                    break;
                }
            }
