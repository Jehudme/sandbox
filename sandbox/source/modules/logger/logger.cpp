...
namespace sandbox::modules {
    ...
    logger::logger(flecs::world& ecs) {
        ...
        svc.execute_command = [](void* inst, uint32_t command_id, const uint8_t* payload, size_t size) {
            auto logger_inst = static_cast<logger*>(inst);
            switch (static_cast<sandbox::schemas::logger::LoggerCommand>(command_id)) {
                case sandbox::schemas::logger::LoggerCommand_Log: {
                    auto args = flatbuffers::GetRoot<sandbox::schemas::logger::LogArgs>(payload);
                    int32_t result = logger_inst->log(payload, size);
                    if (args->out_result_ptr()) {
                        *reinterpret_cast<int32_t*>(args->out_result_ptr()) = result;
                    }
                    break;
                }
                case sandbox::schemas::logger::LoggerCommand_SetProperty: {
                    auto args = flatbuffers::GetRoot<sandbox::schemas::logger::SetPropertyArgs>(payload);
                    logger_inst->set_property(args->key()->c_str(), args->json_value()->c_str());
                    break;
                }
                case sandbox::schemas::logger::LoggerCommand_GetProperty: {
                    auto args = flatbuffers::GetRoot<sandbox::schemas::logger::GetPropertyArgs>(payload);
                    int32_t result = logger_inst->get_property(args->key()->c_str(), reinterpret_cast<sandbox_payload*>(args->out_payload_ptr()));
                    if (args->out_result_ptr()) {
                        *reinterpret_cast<int32_t*>(args->out_result_ptr()) = result;
                    }
                    break;
    ...
    int32_t logger::log(const uint8_t* log_msg_fb, size_t size) {
        ...
        auto log_msg = flatbuffers::GetRoot<sandbox::schemas::logger::LogArgs>(log_msg_fb);
        if (!log_msg) return -1;
