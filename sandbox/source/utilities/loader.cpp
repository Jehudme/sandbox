...
namespace sandbox::internal {
    ...
    std::expected<void, std::string> load(world& ecs, std::string_view virtual_path) {
        ...
        auto absolute_res = fs.absolute(std::string(virtual_path));
        
        if (!absolute_res) {
            return std::unexpected(absolute_res.error() + " for: " + std::string(virtual_path));
        }

        std::string exact_path = *absolute_res;
