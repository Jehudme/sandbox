...
namespace sandbox::modules {
    ...
    static sandbox_requirement_info_t application_requirements[] = {
        { SANDBOX_REQUIREMENT_KIND_MODULE, SANDBOX_REQUIREMENT_STRICTNESS_REQUIRED, "runtime", "sandbox", 1, 0, -1 },
        { SANDBOX_REQUIREMENT_KIND_MODULE, SANDBOX_REQUIREMENT_STRICTNESS_REQUIRED, "filesystem", "sandbox", 1, 0, -1 },
    ...
    static fs::path get_os_appdata_workspace(const std::string& app_name) {
        ...
        base_path = local_app ? fs::path(local_app) : fs::path(std::getenv("USERPROFILE")) / "AppData" / "Local";
    #elif defined(__APPLE__)
        const char* home = std::getenv("HOME");
        base_path = home ? fs::path(home) / "Library" / "Application Support" : fs::temp_directory_path();
        ...
        if (xdg_data && *xdg_data != '\0') {
            ...
            const char* home = std::getenv("HOME");
            base_path = home ? fs::path(home) / ".local" / "share" : fs::temp_directory_path();
        }
    #endif
    ...
    void stage_filesystem(flecs::world &ecs) {
        ...
        fs::path cache_path = get_os_temp_workspace(engine_namespace) / "plugin_cache";
        fs::path saves_path = get_os_appdata_workspace(engine_namespace) / "saves";

        fs::create_directories(cache_path);
        fs::create_directories(saves_path);

        sandbox::modules::filesystem::mount(ecs, cache_path.string().c_str(), "cache://", false);
        sandbox::modules::filesystem::mount(ecs, saves_path.string().c_str(), "save://", false);
    }

    void orchestrate_modules(flecs::world &ecs) {
        try {
            std::vector<std::string> plugins_paths = sandbox::modules::filesystem::list_files(ecs, "app://plugins");

            for (const std::string& plugin_path : plugins_paths) {
                std::filesystem::path path(plugin_path);
                if (path.extension() == NATIVE_LIB_EXTENSION) {
                    try {
                        sandbox::modules::logs::info(ecs, "Loading plugin: {}", path.string());
                        auto library_data = sandbox::modules::filesystem::read_all_bytes(ecs, path.string().c_str());
                        sandbox::core::library_loader::load_from_memory(ecs, library_data);
                    } catch (const std::exception& e) {
                        sandbox::modules::logs::error(ecs, "Failed to load plugin {}: {}", path.string(), e.what());
                    }
                }
            }
        } catch (const std::exception& e) {
            // plugins folder doesn't exist or is invalid
            sandbox::modules::logs::info(ecs, "No plugins loaded (plugins directory not found or error): {}", e.what());
        }
    }

    void ingest_configuration(flecs::world &ecs) {
        try {
            sandbox::modules::logs::trace(ecs, "Checking if app://configuration.json exists...");
            if (sandbox::modules::filesystem::exists(ecs, "app://configuration.json")) {
                ...
                std::string content = sandbox::modules::filesystem::read_all_text(ecs, "app://configuration.json");
                sandbox::modules::logs::trace(ecs, "Read {} bytes. Parsing properties...", content.size());
                if (!content.empty()) {
                ...
                sandbox::modules::logs::trace(ecs, "app://configuration.json does not exist.");
            }
        } catch (const std::exception& e) {
            sandbox::modules::logs::error(ecs, "Failed to ingest configuration: {}", e.what());
        }
    }

    SANDBOX_DECLARE_MODULE(application_t, {
        .name = "application",
