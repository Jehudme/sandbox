...
std::optional<sandbox::properties> parse_cli(int argc, char **argv) {
  ...
  cli_app.add_option("--logs-level", logs_level, "Set log level (e.g., trace, debug, info, warn, error, critical)");
  ...
  cli_app.add_flag("--logs-console,!--no-logs-console", logs_console, "Enable/disable console logging");
  ...
  cli_app.add_flag("--logs-file-truncate,!--no-logs-file-truncate", logs_file_truncate, "Enable/disable file truncation");

  std::optional<bool> logs_file_rotating;
  cli_app.add_flag("--logs-file-rotating,!--no-logs-file-rotating", logs_file_rotating, "Enable/disable file rotation");

  std::optional<int64_t> logs_file_max_size;
  cli_app.add_option("--logs-file-max-size", logs_file_max_size, "Set rotating file max size");

  std::optional<int64_t> logs_file_max_files;
  cli_app.add_option("--logs-file-max-files", logs_file_max_files, "Set rotating max files");

  std::optional<bool> logs_async;
  cli_app.add_flag("--logs-async,!--no-logs-async", logs_async, "Enable/disable async logging");

  std::optional<int64_t> logs_async_queue_size;
  cli_app.add_option("--logs-async-queue-size", logs_async_queue_size, "Set async queue size");

  std::optional<int64_t> logs_async_thread_count;
  cli_app.add_option("--logs-async-thread-count", logs_async_thread_count, "Set async thread count");
  ...
  if (logs_flush_level) engine_properties.set("booting-configuration/logs-flush_level", *logs_flush_level);
  if (logs_pattern) engine_properties.set("booting-configuration/logs-pattern", *logs_pattern);
  if (logs_console) engine_properties.set("booting-configuration/logs-console_enabled", *logs_console);
  if (logs_filepath) engine_properties.set("booting-configuration/logs-filepath", *logs_filepath);
  if (logs_file_truncate) engine_properties.set("booting-configuration/logs-file_truncate", *logs_file_truncate);
  if (logs_file_rotating) engine_properties.set("booting-configuration/logs-file_rotating", *logs_file_rotating);
  if (logs_file_max_size) engine_properties.set("booting-configuration/logs-file_max_size", *logs_file_max_size);
  if (logs_file_max_files) engine_properties.set("booting-configuration/logs-file_max_files", *logs_file_max_files);
  if (logs_async) engine_properties.set("booting-configuration/logs-async_enabled", *logs_async);
  if (logs_async_queue_size) engine_properties.set("booting-configuration/logs-async_queue_size", *logs_async_queue_size);
  if (logs_async_thread_count) engine_properties.set("booting-configuration/logs-async_thread_count", *logs_async_thread_count);
